...
框架目前已经实现了两个自动生成工具（具体操作，参考[编辑器工具](./编辑器工具.md)）
...
- assets
  - resources (框架生成的资源配置表和预制代码，都是直接访问这个目录下的资源)
    - Prefab (预制目录)
      - UI (界面预制，命名P_UI_***)
      - Entity (非界面预制，命名P_Entity_***)
    - CFG_ResPath.bin (资源路径的二进制配置文件)
  - Script (脚本目录)
    - UI (根据界面预制自动生成的界面控制脚本目录)
    - Entity (根据非界面预制自动生成的控制脚本目录)
