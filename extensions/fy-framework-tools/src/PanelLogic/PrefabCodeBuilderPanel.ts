...
export default class PrefabCodeBuilderPanel {
    ...
    public static init(...params: any[]) {
        ...
        let btnGenPrefabCode: any = params[0];
        let pgGenPrefabCode: any = params[1];
     ...
     * @returns 
     */
    public static extractNodeName(nodeName: string): { name: string, components?: string[] } {
    ...
    public static genViewClass(componentInfoList: ComponentInfoList, clsName: string, prefabType: PrefabCodeBuilderEnum.PrefabType, scriptPath: string): string | undefined {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (components) {
                ...
                for (let j = 0; j < cLen; j++) {
                    ...
                    let componentEventConfig = PrefabCodeBuilderConst.COMPONENT_EVENT_CONFIG[componentType];
        ...
        viewTemplate = viewTemplate.replace(PrefabCodeBuilderConst.VIEW_KEY_VARIABLE_ASSIGNMENT, variableAssignment);
        viewTemplate = viewTemplate.replace(PrefabCodeBuilderConst.VIEW_KEY_EVENT_ON, eventOn);
        viewTemplate = viewTemplate.replace(PrefabCodeBuilderConst.VIEW_KEY_EVENT_OFF, eventOff);
        viewTemplate = viewTemplate.replace(PrefabCodeBuilderConst.VIEW_KEY_EVENT_CALLBACK, eventCallback);
    ...
    public static genModelClass(componentInfoList: ComponentInfoList, clsName: string, prefabType: PrefabCodeBuilderEnum.PrefabType): string | undefined {
        ...
        modelTemplate = modelTemplate.replace(new RegExp('\\' + PrefabCodeBuilderConst.VIEW_KEY_CLASS_NAME, 'g'), className);
        modelTemplate = modelTemplate.replace(new RegExp('\\' + PrefabCodeBuilderConst.VIEW_KEY_PREFAB_NAME, 'g'), prefabName);
    ...
    public static genControllerClass(componentInfoList: ComponentInfoList, clsName: string, prefabType: PrefabCodeBuilderEnum.PrefabType): string | undefined {
        ...
        controllerTemplate = controllerTemplate.replace(new RegExp('\\' + PrefabCodeBuilderConst.VIEW_KEY_CLASS_NAME, 'g'), className);
        controllerTemplate = controllerTemplate.replace(new RegExp('\\' + PrefabCodeBuilderConst.VIEW_KEY_PREFAB_NAME, 'g'), prefabName);
     ...
     * @returns 
     */
    public static genPrefabClass(prefabFileName: string, prefabType: PrefabCodeBuilderEnum.PrefabType) {
