...
export enum JoystickEvent {
  TOUCH_START = "Joystick-Touch-Start",
  TOUCH_MOVE = "Joystick-Touch-Move",
  TOUCH_END = "Joystick-Touch-End"
}
 ...
 */
@ccclass("Joystick")
export class Joystick extends FYComponent {
  @property({
    type: Node,
    displayName: "Dot",
    tooltip: "摇杆操纵点",
   ...
   */
  onEnable() {
    FY.event.on(SET_JOYSTICK_TYPE, this._onSetJoystickType, this);
  }
   ...
   */
  onDisable() {
    FY.event.off(SET_JOYSTICK_TYPE, this._onSetJoystickType, this);
  }
  ...
  _touchStartEvent(event: EventTouch) {
    ...
    FY.event.emit(JoystickEvent.TOUCH_START, event);
    // 这里获取到的UI坐标，原点在屏幕左下角，UI制作时，如果原点设置成屏幕中间，需要自行转换
  ...
  _touchMoveEvent(event: EventTouch) {
    ...
    FY.event.emit(JoystickEvent.TOUCH_MOVE, event, {
      speedType,
  ...
  _touchEndEvent(event: EventTouch) {
    ...
    FY.event.emit(JoystickEvent.TOUCH_END, event, {
